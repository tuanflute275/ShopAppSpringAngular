...
export class CartComponent implements OnInit{
  ...
  handleCoupon() {
    ...
    this.couponService.applyCoupon(coupon, this.totalCart).subscribe(response => {
      console.log(response.data)
      if(response.data == 'Coupon not found' || response.data == 'Coupon is not active'){
        ...
        this.discount = Math.round(this.totalCart - Math.round(response.data))
  ...
  getAll(){
    this.cartService.findAll().subscribe(response=>{
      ...
      const sumWithInitial = arrayTotal.reduce(
        (accumulator:any, currentValue: any) => accumulator + currentValue, initialValue,
      );
  ...
  updateQuantity(item: any, e: any, totalAmout: number) {
    ...
    this.cartService.update(item.cartId, item.product.productId, this.quantity, totalAmout).subscribe((response)=>{
      this.getAll()
    })
  }
  ...
  handleMinus(item: any, quantity: number, totalAmout: number) {
    ...
    this.cartService.update(
      item.cartId, item.product.productId, this.quantity,
      (item.product.productSalePrice > 0 ? item.product.productSalePrice : item.product.productPrice) * this.quantity)
      .subscribe((response)=>{
        this.getAll()
    })
  }
